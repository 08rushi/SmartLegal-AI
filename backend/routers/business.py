...
async def list_applications(
    ...
    async with db.execute(
        """SELECT id, business_type, service, status, notes, created_at, updated_at
           FROM business_applications WHERE user_id = ? ORDER BY created_at DESC""",
        (current_user["id"],)
    ) as cur:
...
async def get_application(
    ...
    """
    async with db.execute(
        "SELECT * FROM business_applications WHERE id = ?",
        (app_id,)
    ) as cur:
...
async def update_application(
    ...
    await db.execute(
        "UPDATE business_applications SET status = ?, notes = ?, updated_at = ? WHERE id = ?",
...
async def delete_application(
    ...
    await db.execute("DELETE FROM business_checklist_items WHERE application_id = ?", (app_id,))

    # Delete application
    await db.execute("DELETE FROM business_applications WHERE id = ?", (app_id,))
    await db.commit()
...
async def get_checklist(
    ...
    # Verify ownership
    async with db.execute(
        "SELECT user_id FROM business_applications WHERE id = ?",
        (app_id,)
    ) as cur:
        app = await cur.fetchone()
    ...
    async with db.execute(
        "SELECT id, item_text, is_done, updated_at FROM business_checklist_items WHERE application_id = ?",
        (app_id,)
    ) as cur:
...
async def save_checklist(
    ...
    for item in data.items:
        if item.id:
            await db.execute(
                "UPDATE business_checklist_items SET is_done = ?, updated_at = ? WHERE id = ? AND application_id = ?",
                (1 if item.is_done else 0, now, item.id, app_id),
            )
        else:
            await db.execute(
                "UPDATE business_checklist_items SET is_done = ?, updated_at = ? WHERE application_id = ? AND item_text = ?",
