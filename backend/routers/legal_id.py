...
async def list_applications(
    ...
    rows = await db.fetch(
        """SELECT id, id_type, service, status, notes, created_at, updated_at
           FROM id_applications WHERE user_id = $1 ORDER BY created_at DESC""",
        current_user["id"],
    )
...
async def get_application(
    ...
    app = await db.fetchrow(
        "SELECT * FROM id_applications WHERE id = $1",
        app_id,
    )
...
async def update_application(
    ...
    await db.execute(
        "UPDATE id_applications SET status = $1, notes = $2, updated_at = $3 WHERE id = $4",
...
async def delete_application(
    ...
    await db.execute("DELETE FROM id_checklist_items WHERE application_id = $1", app_id,)

    # Delete application
    await db.execute("DELETE FROM id_applications WHERE id = $1", app_id,)
...
async def get_checklist(
    ...
    # Verify ownership
    app = await db.fetchrow(
        "SELECT user_id FROM id_applications WHERE id = $1",
        app_id,
    )
    ...
    items = await db.fetch(
        "SELECT id, item_text, is_done, updated_at FROM id_checklist_items WHERE application_id = $1",
        app_id,
    )
...
async def save_checklist(
    ...
    for item in data.items:
        if item.id:
            await db.execute(
                "UPDATE id_checklist_items SET is_done = $1, updated_at = $2 WHERE id = $3 AND application_id = $4",
                1 if item.is_done else 0, now, item.id, app_id,
            )
        else:
            await db.execute(
                "UPDATE id_checklist_items SET is_done = $1, updated_at = $2 WHERE application_id = $3 AND item_text = $4",
