...
import type { PropertyGuidance, ChecklistItem } from '../types'
...
export default function PropertyDetail() {
  ...
  useEffect(() => {
    ...
    dispatch(fetchPropertyGuidance(propertyType))
    if (token) {
      dispatch(fetchPropertyApplications())
    }
  }, [mounted, dispatch, propertyType, token])

  useEffect(() => {
    setChecklistItems(currentChecklist)
  }, [currentChecklist])

  const userAppsForThisProperty = applications.filter((app) => app.property_type === propertyType)
  ...
  const handleCreateApplication = async () => {
    ...
    try {
      ...
      if (createPropertyApplication.fulfilled.match(resultAction)) {
  ...
  const handleViewChecklist = async (appId: string) => {
    ...
    const resultAction = await dispatch(fetchPropertyChecklist(appId))
    if (fetchPropertyChecklist.rejected.match(resultAction)) {
  ...
  const handleSaveChecklist = async () => {
    ...
    try {
      ...
      if (savePropertyChecklist.fulfilled.match(resultAction)) {
        alert('Checklist saved successfully')
      } else if (savePropertyChecklist.rejected.match(resultAction)) {
    ...
    return (
      <div className="content-wrap py-5 sm:py-6">
        <div className="section-card mx-auto max-w-7xl rounded-[32px] p-6 sm:p-8">
          ...
          <p className="text-white/60">Property type not found.</p>
        </div>
      </div>
    )
  }

  const guidance: PropertyGuidance = currentGuidance
        ...
        <div className="space-y-6 mb-12 xl:mb-0">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.02] backdrop-blur-sm p-6">
            <div className="mb-4 h-28 overflow-hidden rounded-xl border border-white/10 bg-white/[0.02]">
              <ServiceArt hub="property" serviceKey={propertyType || ''} preserve="xMidYMid meet" />
            </div>
